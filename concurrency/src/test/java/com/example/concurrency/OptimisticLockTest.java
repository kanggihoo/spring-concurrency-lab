...
class OptimisticLockTest {
    ...
    @Test
    @DisplayName("Optimistic lock - single reservation returns 200 OK")
    void optimisticLock_single_reservation_returns_200() {
        // given
        ...
        ResponseEntity<String> response = restTemplate.postForEntity(
                "/api/reservations/optimistic", request, String.class);
    ...
    @Test
    @DisplayName("Optimistic lock - returns 409 when sold out")
    void optimisticLock_returns_409_when_sold_out() {
    ...
    @Test
    @DisplayName("Optimistic lock - 100 concurrent threads, data integrity with retry")
    void optimisticLock_service_100_concurrent_threads_integrity() throws InterruptedException {
        int threadCount = 100;
        ...
        System.out.println("[Optimistic Lock - Service] Success: " + successCount.get()
                + ", Fail(retry exhausted): " + failCount.get()
                + ", Stock: " + concert.getStock()
                + ", Reservations: " + reservationCount);

        // integrity check: success + remaining stock = 100
        // some threads may fail due to retry exhaustion, but integrity must hold
    ...
    @Test
    @DisplayName("Optimistic lock - 100 concurrent API requests, data integrity with retry")
    void optimisticLock_api_100_concurrent_requests_integrity() throws InterruptedException {
        int threadCount = 100;
        ExecutorService executor = Executors.newFixedThreadPool(threadCount);
        CountDownLatch latch = new CountDownLatch(threadCount);
        AtomicInteger successCount = new AtomicInteger(0);
        ...
        System.out.println("[Optimistic Lock - API] Success: " + successCount.get()
                + ", Fail(retry exhausted): " + failCount.get()
                + ", Stock: " + concert.getStock()
                + ", Reservations: " + reservationCount);
