...
const optimisticLockExhaustedCounter = new Counter(
  "reservation_optimistic_exhausted_count",
);
const pessimisticLockTimeoutCounter = new Counter(
  "reservation_pessimistic_timeout_count",
);
const connectionExhaustedCounter = new Counter(
  "reservation_connection_exhausted_count",
);
  ...
  scenarios: {
    spike_test: {
      ...
      stages: [
        ...
        // { duration: "30s", target: 300 }, // 경합 강도 증가
        // { duration: "15s", target: 800 }, // 순간 spike
        // { duration: "5s", target: 0 },
      ],
      // 이 태그가 프로메테우스의 Label 로 들어가서 그라파나에서 필터링이 가능해집니다.
...
export default function () {
  const payload = JSON.stringify({
    ...
    userId: __VU * 100000 + __ITER, // 요청마다 고유 유저 아이디 생성
  });
  ...
  if (res.status === 200) {
    ...
    connectionExhaustedCounter.add(1); // 커넥션 풀 고갈
