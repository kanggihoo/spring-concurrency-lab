...
- Pessimistic Lock pool size별 처리량, row lock wait, p95/p99
- Pessimistic Lock `lock_timeout` 적용 시 HTTP 응답 분포와 p95/p99
...
- [x] Hikari summary는 k6 본 실행 구간 기준으로 재생성했고, pool size와 Hikari max 값의 일관성을 검증했다.
